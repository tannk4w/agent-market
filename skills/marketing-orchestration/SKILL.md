...
- CMS/browser/scheduling operations that do not have a matching specialist or platform adapter. Blog/API publishing is supported only through `marketing-blog-publish` after a completed content package exists and the user explicitly requests upload/publish.
...
### Handoff to `marketing-image`

When images are requested or required after the written package is ready, use this template:

```markdown
Use skill: marketing-image

Content title:
Content language:
Platform/use case: [blog thumbnail / cover / inline article image / social visual]
Audience:
Article angle / reader promise:
Sections needing inline images:
Visual tone:
Brand constraints:
Text overlay, if any:
Image quantity and aspect ratio:
Generation requested: yes/no
Publish later: yes/no

Success criteria:
- Prompts follow `marketing-image` 8-component formula.
- If generation is requested, call `image_generate` and return real paths/URLs only after the tool succeeds.
- If publish later is yes, return an image manifest with IDs, local paths, alt text, captions, and placements.
```

### Handoff to `marketing-blog-publish`

When handing work to `marketing-blog-publish` (after blog content and any image manifest are ready), use this adapter-neutral template:

```markdown
Use skill: marketing-blog-publish

Platform:
Publish intent: [prepare_only / draft / publish]
Adapter to load: references/platforms/[platform].md

Title:
Slug:
Content format available: [markdown/html]
Content markdown:
Content HTML:
Excerpt:
...
Author ID, if known:
Image manifest:
- Thumbnail: [local path, alt, caption]
- Inline images: [IDs, local paths, placements, alt, captions]
Authentication source: [environment variable name only, never secret value]

## Success Criteria
- Load and follow the platform adapter before preparing payloads or calling APIs.
- If uploading/publishing, return platform, adapter used, ID/URL/status when available.
- Upload/map images according to the adapter; do not embed local image paths in final published HTML unless the adapter supports it.
- Handle errors: auth errors, validation errors, server errors, duplicate slug, image upload failures.

Do not:
- Expose API keys or secret values.
- Publish when the user only asked for draft/content creation.
- Invent missing platform API rules if no adapter exists.
```
...
- Do not add unnecessary explanation to simple edits or narrow requests; still follow the required marketing workflow steps when the request is marketing/content/creative writing.
